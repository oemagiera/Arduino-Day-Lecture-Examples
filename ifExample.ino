//basic if button clicked do this example.
...
uint8_t DCMotor[] = {0x6c, 0xc8, 0x40, 0x87, 0xc8, 0xd8}; // MAC address of DC Motor, not connected in physical demo
...
{
  //saying send data starting at this memory address that will go to this one (depending on the length) to this device
  data.State = state;
  data.device = 0;
  ...
  esp_err_t result5 = esp_now_send(solder2, (uint8_t *) &data, sizeof(data));
}
...
void setup() {
  ...
  memcpy(peerInfo5.peer_addr, solder2, 6);
  peerInfo5.channel = 0;
